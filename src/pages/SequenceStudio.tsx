...
export default function SequenceStudio() {
  ...
  const handleGenerateFullSequence = async () => {
    ...
    if (!step1?.content || !step2?.content) {
      toast.error("Day 1 Email and SMS must have content first. Generate them individually or they should come from Personalization Studio.");
      return;
    }
    ...
    const followupStepIds = ['step-3', 'step-4', 'step-5'];
      ...
      if (!error && data?.sequence_steps) {
        ...
        setSequenceSteps(prev => prev.map(step => {
          const generated = generatedSteps.find((g: { day: number; channel: string }) => 
            g.day === step.day && g.channel === step.channel && step.type === 'followup'
          );
        ...
        toast.success("Full sequence generated! Review in the QA tab.");
        setActiveTab("qa");
      } else {
...
Best,`,
      ...
      if (step.day === 5 && step.channel === 'sms') {
        return {
          ...step,
          content: `Dr. ${lastName}, quick follow-up on the ${job?.state || ''} ${job?.specialty} role at $${payRate}/hr. Still interested?`.substring(0, 160),
        };
      }
...
${emailSender.split('@')[0]}`;
  ...
  const hasInitialContent = sequenceSteps.find(s => s.id === 'step-1')?.content && 
                            sequenceSteps.find(s => s.id === 'step-2')?.content;
